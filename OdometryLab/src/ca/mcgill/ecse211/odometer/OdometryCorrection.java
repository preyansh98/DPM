			...
			if (lightPoll < LineIntensity) {
			   ...
			   boolean pathComplete = false; 
				if (theta > -45 && theta < 45) { 
					...
					// should be at 90
					 odometer.setXYT(SquareTile*counterXR, SquareTile*counterYF, theta); //the y should be constant from the previous case
					...
					odometer.setXYT(SquareTile*counterXR, SquareTile*counterYF, theta); //y should be changing 
					counterYF--; //the lines are reducing
					
				} else if (theta > 225 && theta < 315) {
					// should be at 270
					//now its coming back to original position
					// x should be reducing and y should be 0 
					odometer.setXYT(SquareTile*counterXR, 0, theta);
					counterXR--; 
					pathComplete = true; 
				}
			}
